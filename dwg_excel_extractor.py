...
def extract_excel_info(excel_source):
    ...
    if "PAGE 3" in wb.sheetnames:
        ...
        if "PAGE 4" in wb.sheetnames:
            ...
            global_dc_loss = None
            try:
                if ws4["B9"].value is not None:
                    global_dc_loss = float(ws4["B9"].value)
            except Exception:
                pass

            arr1_l = float(ws4["B12"].value or 2.37)
            arr2_l = float(ws4["D12"].value or 1.35)
            arr3_l = float(ws4["B16"].value or 0.0)
            arr4_l = float(ws4["D16"].value or 0.0)

            if global_dc_loss is None or global_dc_loss == 0.0:
                valid_losses = [l for l in [arr1_l, arr2_l, arr3_l, arr4_l] if l > 0]
                if valid_losses:
                    global_dc_loss = sum(valid_losses) / len(valid_losses)
                else:
                    global_dc_loss = 2.00

            ex_data["dc_losses"] = {
                "global_loss": float(global_dc_loss),
                "arr1_res": int(round(float(ws4["B11"].value or 292))),
                "arr1_loss": arr1_l,
                "arr2_res": int(round(float(ws4["D11"].value or 233))),
                "arr2_loss": arr2_l,
                "arr3_res": int(round(float(ws4["B15"].value or 146))),
                "arr3_loss": arr3_l,
                "arr4_res": int(round(float(ws4["D15"].value or 0))),
                "arr4_loss": arr4_l,
            }
    ...
    if ex_data["produced_energy_kwh"] == 0.0 or not ex_data.get("balances_table") or len(ex_data.get("balances_table", [])) < 12:
        ...
        ex_data["solar_fraction_sf"] = round((tot_es / tot_eu) * 100, 2) if tot_eu > 0 else 45.05
        ...
        if not ex_data.get("loss_diagram"):
            ...
            ex_data["loss_diagram"] = {
                ...
                "final_energy": avail,
                "efr_grid": tot_ef,
                "battery_stored": round(tot_eu * 0.267, 1) if ex_data.get("has_battery") else 0,
                "direct_use": tot_es
            }

        if not ex_data.get("p50_p90"):
            v_n = float(ex_data.get("weather_data_variance") or 3.8)
            cv_n = float(ex_data.get("custom_variability") or 5.7)
            var_kwh = round(tot_ea * 0.059)
            ...
            ex_data["p50_p90"] = {
                ...
                "variability": var_kwh,
                "p50": p50_kwh,
                "p90": round(p50_kwh - 0.674 * var_kwh),
                "p75": round(p50_kwh - 1.282 * var_kwh)
            }
    ...
    for p_idx, page in enumerate(doc):
        ...
        if p_idx >= 3 and "Loss diagram" in p_txt and ("PV nominal" in p_txt or "Array virtual energy" in p_txt or "PV conversion" in p_txt):
            ...
            if has_bat:
                ...
                add_redact(pymupdf.Rect(115, 698, 150, 712))
                ...
                add_text((88.0 - w_ef, 708.5), ef_str, fontsize=7.5, fontname="helv", color=(0, 0, 0))
                add_text((147.0 - w_bs, 708.5), bs_str, fontsize=7.5, fontname="helv", color=(0, 0, 0))
                add_text((201.0 - w_du, 708.5), du_str, fontsize=7.5, fontname="helv", color=(0, 0, 0))
            else:
        ...
        if p_idx >= 3 and "P50 - P90 evaluation" in p_txt and "Annual production probability" in p_txt:
            ...
            for w in page.get_text("words"):
                if 180 < w[1] < 290 and (w[0] > 480 or (220 < w[0] < 250)):
                    if any(c.isdigit() for c in w[4]) and w[4] not in ["0.0", "1.0", "5.9"]:
                        add_redact(pymupdf.Rect(w[0] - 1, w[1] - 1, w[2] + 1, w[3] + 1))
            ...
            add_text((511.8 - w_pv, 250.5), str(p_var_n), fontsize=7.5, fontname="helv", color=(0, 0, 0))

            w_p50 = pymupdf.get_text_length(str(p50_n), fontname="helv", fontsize=7.5)
            add_text((511.8 - w_p50, 262.5), str(p50_n), fontsize=7.5, fontname="helv", color=(0, 0, 0))

            w_p90 = pymupdf.get_text_length(str(p90_n), fontname="helv", fontsize=7.5)
            add_text((511.8 - w_p90, 274.5), str(p90_n), fontsize=7.5, fontname="helv", color=(0, 0, 0))

            w_p75 = pymupdf.get_text_length(str(p75_n), fontname="helv", fontsize=7.5)
            add_text((511.8 - w_p75, 286.5), str(p75_n), fontsize=7.5, fontname="helv", color=(0, 0, 0))
